...
void refresh_OLED( void )
{
    ...
    // --- Display Resistance (e.g., on Page 2) ---
    ...
    */
   oled_SetCursor(2, 0); // Set cursor to Page 2, Column 0

    int i = 0;
    while (Buffer[i] != '\0' && i < 16)
    {
        for (int j = 0; j < 8; j++) // Send 8 bytes of font data
        {
    ...
    /* Buffer now contains your character ASCII codes for LED Display
       ...
       - for each c = ASCII code = Buffer[0], Buffer[1], ...,
           send 8 bytes in Characters[c][0-7] to LED Display
    */
   oled_SetCursor(4, 0); // Set cursor to Page 4, Column 0
...
/**
 * @brief Sets the display cursor to a specific page (row) and column.
 * @param page The page (line) to write to (0-7).
 * @param col The column (pixel) to start at (0-127).
 */
static void oled_SetCursor(unsigned char page, unsigned char col)
{
    if (page > 7) page = 7;
...
/**
 * @brief Waits for a specified number of milliseconds using TIM3.
 * @param ms The number of milliseconds to wait.
 */
static void delay_tim3_ms(uint32_t ms)
{
    TIM3->CNT = 0; // Reset timer count
    while(TIM3->CNT < ms); // Wait for count to reach ms
}
